...
6. 如果现有规则经常不生效，先检查 CLAUDE.md 是否过长、重复、措辞歧义，或关键规则被埋没；不要默认继续堆加新规则。
7. 除非用户明确要求覆盖原文件，否则生成一个新的对比文件或最小 diff，方便用户确认后再替换。
...
CLAUDE.md 可以用 `@path/to/file` 引用其他文件，但引用内容仍会占用上下文。只有当内容稳定、经常相关、且拆分后能让主文件更清晰时，才建议使用引用；偶尔才相关的长流程或领域知识，优先沉淀为 skill。

`IMPORTANT`、`YOU MUST` 等强调词只用于少数关键规则。滥用强调会稀释信号，让真正重要的约束更难被识别。

## 放置层级与生效排查

评审 CLAUDE.md 时，不只判断内容是否值得保留，还要判断它应放在哪一层。优先把规则放在能覆盖目标场景的最窄层级，避免把低频或局部规则堆进顶层文件。

| 放置位置 | 适合内容 | 注意事项 |
|---|---|---|
| `~/.claude/CLAUDE.md` | 适用于所有 Claude 会话的个人偏好 | 不应写项目专属规则 |
| `项目根目录/CLAUDE.md` | 项目共享规则，可与他人共享 | 建议提交到git，只写对团队和项目长期有效的规则 |
| `项目根目录/CLAUDE.local.md` | 个人项目偏好 | 应加入 `.gitignore`，不与他人共享 |
| 父目录 `CLAUDE.md` | monorepo 或多项目共享规则，例如仓库根目录规则会与子项目规则一起加载 | 避免与子项目规则重复或冲突 |
| 子目录 `CLAUDE.md` | 只适用于某个子目录、服务、模块或技术栈的特化规则 | Claude 在处理该目录文件时才会按需加载 |

规则不生效时，除了检查过长、重复和歧义，也要检查当前会话实际加载的说明文件、当前工作目录、上下级 CLAUDE.md、CLAUDE.local.md 和 `@path` 引入链，确认是否被更近或更具体的规则覆盖。
...
| 规则不生效就继续加规则 | 先压缩、去重、消歧义，并突出真正关键的约束 |
| 大量使用强调词 | 只给少数高代价规则加重信号 |
...
4. **建议改写版**：优先给最小可落地 diff；如变化较大，再内联展示完整新版或保存为新文件。
